# client.py
"""
Client WebSocket + Tkinter pour le projet de chat.
Usage:
    python client.py
Champs à remplir dans l'UI: Host, Port, Username puis Connect.
"""
...
def send_action(action, payload=None):
    ...
    if payload is None:
        payload = {}
    out_queue.put({"action": action, "payload": payload})
...
async def network_loop(uri, username):
    ...
    try:
        async with websockets.connect(uri, ping_interval=20, ping_timeout=10) as ws:
            ...
            # Demander la liste des salons initiale
            await ws.send(json.dumps({"action": "list_rooms", "payload": {}}))
            ...
            try:
                while True:
                    # envoi si il y a des objets dans out_queue
                    try:
        ...
        # erreur de connexion initiale ou perte
...
class ChatClientUI:
    ...
    def on_connect(self):
        ...
        # on demande une refresh dans quelques secondes (le réseau remplira in_queue)
    ...
    def check_connected_status(self):
        if connected:
            ...
            send_action("list_rooms", {})
        else:
            # si pas connecté après 2s => affiche erreur (la boucle réseau mettra l'erreur)
    ...
    def create_room_prompt(self):
        ...
        if room:
            send_action("create_room", {"room": room})
    ...
    def join_selected_room(self, event=None):
        ...
        send_action("join_room", {"room": room})

    def leave_room(self):
        if not connected:
            messagebox.showerror("Erreur", "Connecte-toi d'abord.")
            return
        send_action("leave_room", {})
    ...
    def send_message(self):
        ...
        if not text:
            return
        send_action("send_message", {"message": text})
    ...
    def poll_incoming(self):
        while not in_queue.empty():
            ...
            if action == "rooms_list":
                ...
                # afficher éventuellement pop-up pour erreurs critiques
                if reason in ("username_taken", "unable_to_connect"):
                    messagebox.showerror("Erreur serveur", f"{reason}\n{detail}")

            else:
                # messages inconnus : on affiche pour debug
